...
export const Blackjack = ({ onBack }: { onBack: () => void }) => {
  ...
  } as const; // ✅ CORREÇÃO APLICADA AQUI
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-slate-800/80 backdrop-blur-lg rounded-2xl shadow-2xl border border-slate-700 p-4 sm:p-6 md:p-8">
          <div className="text-center mb-6">
            ...
            <p className="text-slate-400">Your Balance: <span className="font-bold text-amber-300">{profile?.balance || 0}</span> credits</p>
          </div>
          {gameState === 'betting' ? (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 max-w-sm mx-auto">
              <div>
                <label className="block text-slate-300 mb-2 text-center text-lg">Bet Amount: <span className="font-bold text-amber-400 text-xl">{betAmount}</span></label>
                <input
                  type="range"
                  min="10"
            ...
            <div className="space-y-6">
              <div className="bg-green-900/20 rounded-t-full border-4 border-amber-800/50 p-6 min-h-[220px]">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-xl font-semibold text-slate-300">Dealer's Hand</h3>
                  {gameState === 'finished' && (
              ...
              <div className="bg-green-900/20 rounded-b-full border-4 border-amber-800/50 p-6 min-h-[220px]">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-xl font-semibold text-slate-300">Your Hand</h3>
                  <span className="text-lg text-amber-400 font-semibold bg-slate-900/50 px-3 py-1 rounded-md">
              ...
              <div className="h-24 flex flex-col items-center justify-center">
                ...
                {gameState === 'playing' && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { delay: 0.5 } }} className="flex gap-4 w-full max-w-md">
                    <button onClick={hit} className="flex-1 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white font-bold py-3 rounded-xl transition-all shadow-lg text-lg uppercase tracking-wider">Hit</button>
                    <button onClick={() => stand()} className="flex-1 bg-gradient-to-r from-rose-500 to-red-600 hover:from-rose-600 hover:to-red-700 text-white font-bold py-3 rounded-xl transition-all shadow-lg text-lg uppercase tracking-wider">Stand</button>
                  </motion.div>
                )}
                {gameState === 'finished' && (
                  <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1, transition: { delay: 0.5 } }} className="w-full max-w-md">
                    <button onClick={newGame} className="w-full bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-slate-900 font-bold py-3 rounded-xl transition-all shadow-lg text-lg uppercase tracking-wider">New Game</button>
                  </motion.div>
                )}
              </div>
            </div>
